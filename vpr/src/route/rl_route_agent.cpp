...
void RLRouteAgent::do_action() {
    ...
    if (itry_ > 1) {
        ...
        VTR_LOG("Action %d selected. Changing pres_fac from %.5f to ", action_index, pres_fac_data_.pres_fac_);
...
void RLRouteAgent::update_after_sink_route(const RouteTree& tree, RRNodeId sink_node, const RouterStats& stats) {
#if RLROUTE_IMPL <= 11
    if (itry_ > 1)
        pres_fac_data_.update_after_sink_route(tree, sink_node, prev_cong_, prev_length_, k_step_size, length_fac_);
#elif (RLROUTE_IMPL >= 12 && RLROUTE_IMPL <= 16)
    astar_fac_data_.update_after_sink_route(tree, sink_node, prev_cong_, prev_length_, stats, k_step_size_, prev_heap_ops_, itry_, length_fac_);
#endif
}
...
int astar_fac_data::calculate_reward(const RouteTree& tree,
    ...
    for (const RouteTreeNode& node : tree.root().child_nodes()) {
        if (node.re_expand) {
            for (const RouteTreeNode& child : node.child_nodes()) {
