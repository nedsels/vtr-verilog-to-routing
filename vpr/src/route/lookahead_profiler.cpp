...
LookaheadProfiler::LookaheadProfiler() {
    ...
    if (!lookahead_verifier_csv.is_open()) {
        ...
        throw vtr::VtrError("Could not open lookahead_verifier_info.csv", "lookahead_profiler_error.txt", 1);
    }
...
void LookaheadProfiler::record(const int iteration,
    ...
    int delta_x, delta_y;
    util::get_xy_deltas(curr_inode, sink_inode, &delta_x, &delta_y);
    ...
    if (net_id != ParentNetId::INVALID() && target_net_pin_index != OPEN) {
        ...
        auto tile_type = get_physical_tile_type(cluster_block_id);
